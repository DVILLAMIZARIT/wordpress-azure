 ...
 */
add_filter( 'wp_nav_menu_args', 'bddex_modify_arguments', 100 );
function bddex_modify_arguments( $arguments ) {
    $arguments['walker']          = new HeroMenuWalker();
    return $arguments;
}
add_action('wp_update_nav_menu_item', 'bddex_custom_nav_update',10, 3);
function bddex_custom_nav_update($menu_id, $menu_item_db_id, $args ) {
 ...
 */
add_filter( 'wp_setup_nav_menu_item','bddex_custom_nav_item' );
function bddex_custom_nav_item($menu_item) {
    $fields = array('submenu_type','dropdown_type','widget_area','hide_link','bg_image','bg_image_attachment','bg_image_size','bg_image_position','bg_image_repeat','bg_color','menu_icon');
    foreach($fields as $i=>$field){
...
add_action( 'admin_enqueue_scripts','bddex_add_js_mega_menu');
function bddex_add_js_mega_menu(){
    wp_enqueue_script( 'set_background', AVAS_PLUGIN_URL . '/assets/js/mega-menu.min.js', array( 'jquery', 'jquery-ui-sortable' ), false, true );
    wp_enqueue_style('font-awesome', AVAS_PLUGIN_URL . '/assets/css/font-awesome.min.css');
    wp_enqueue_media();
    add_thickbox();
}

add_filter( 'wp_edit_nav_menu_walker', 'bddex_custom_nav_edit_walker',10,2 );
function bddex_custom_nav_edit_walker($menu_id) {
...
function start_el( &$output, $item, $depth = 0, $args = array(), $current_object_id = 0 ) {
    ...
    if ( ! empty( $item->_invalid ) ) {
        ...
        $title = sprintf( __( '%s (Invalid)', 'avas-core'), $item->title );
    } elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
        $classes[] = 'pending';
        /* translators: %s: title of menu item in draft status */
        $title = sprintf( __('%s (Pending)', 'avas-core'), $item->title );
    }
    ...
    <li data-menuanchor="" id="menu-item-<?php echo esc_attr( $item_id ); ?>" class="<?php echo implode(' ', $classes ); ?>">
            ...
            <dt class="menu-item-handle">
                ...
                <span class="item-controls">
                    ...
                    <span class="item-order hide-if-js">
                        ...
                        ?>" class="item-move-up"><abbr title="<?php esc_attr_e('Move up','avas-core'); ?>">&#8593;</abbr></a>
                        |
                        <a href="<?php
                            echo wp_nonce_url(
                                add_query_arg(
                        ...
                        ?>" class="item-move-down"><abbr title="<?php esc_attr_e('Move down','avas-core'); ?>">&#8595;</abbr></a>
                    </span>
                    <a class="item-edit" id="edit-<?php echo esc_attr( $item_id ); ?>" title="<?php esc_attr_e('Edit Menu Item','avas-core'); ?>" href="<?php
                        echo ( isset( $_GET['edit-menu-item'] ) && $item_id == $_GET['edit-menu-item'] ) ? admin_url( 'nav-menus.php' ) : add_query_arg( 'edit-menu-item', $item_id, remove_query_arg( $removed_args, admin_url( 'nav-menus.php#menu-item-settings-' . $item_id ) ) );
                    ?>"><?php _e( 'Edit Menu Item' ,'avas-core'); ?></a>
                </span>
            </dt>
        </dl>
        ...
        <div class="menu-item-settings" id="menu-item-settings-<?php echo esc_attr( $item_id ); ?>">
            ...
            <p class="description description-thin">
                <label for="edit-menu-item-title-<?php echo esc_attr( $item_id ); ?>">
                    <?php _e( 'Navigation Label' ,'avas-core'); ?><br />
                    <input type="text" id="edit-menu-item-title-<?php echo esc_attr( $item_id ); ?>" class="widefat edit-menu-item-title" name="menu-item-title[<?php echo esc_attr( $item_id ); ?>]" value="<?php echo esc_attr( $item->title ); ?>" />
                </label>
            </p>
            <p class="description description-thin">
                <label for="edit-menu-item-attr-title-<?php echo esc_attr( $item_id ); ?>">
                    <?php _e( 'Title Attribute','avas-core' ); ?><br />
            ...
            <p class="field-link-target description">
                <label for="edit-menu-item-target-<?php echo esc_attr( $item_id ); ?>">
                    ...
                    <?php _e( 'Open link in a new window/tab' ,'avas-core'); ?>
                </label>
            </p>
            <p class="field-css-classes description description-thin">
                <label for="edit-menu-item-classes-<?php echo esc_attr( $item_id ); ?>">
                    <?php _e( 'CSS Classes (optional)' ,'avas-core'); ?><br />
                    <input type="text" id="edit-menu-item-classes-<?php echo esc_attr( $item_id ); ?>" class="widefat code edit-menu-item-classes" name="menu-item-classes[<?php echo esc_attr( $item_id ); ?>]" value="<?php echo esc_attr( implode(' ', $item->classes ) ); ?>" />
                </label>
            </p>
            <p class="field-xfn description description-thin">
                <label for="edit-menu-item-xfn-<?php echo esc_attr( $item_id ); ?>">
                    <?php _e( 'Link Relationship (XFN)' ,'avas-core'); ?><br />
                    <input type="text" id="edit-menu-item-xfn-<?php echo esc_attr( $item_id ); ?>" class="widefat code edit-menu-item-xfn" name="menu-item-xfn[<?php echo esc_attr( $item_id ); ?>]" value="<?php echo esc_attr( $item->xfn ); ?>" />
                </label>
            </p>
            <p class="field-description description description-wide">
                <label for="edit-menu-item-description-<?php echo esc_attr( $item_id ); ?>">
                    <?php _e( 'Description' ,'avas-core'); ?><br />
                    ...
                    <span class="description"><?php _e('The description will be displayed in the menu if the current theme supports it.','avas-core'); ?></span>
                </label>
            </p>
            <?php
            /*
			...
			if ( ! $depth ) {
				...
				<p class="description description-thin">
					...
					<label for="edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>">
						<select id="edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>" class=" <?php echo esc_attr( $key ); ?>" name="<?php echo esc_attr( $key . "[" . $item_id . "]" ); ?>">
							<option value="standard" <?php echo ( $value == 'standard' ) ? ' selected="selected" ' : ''; ?>><?php esc_html_e( 'Standard Dropdown', 'avas-core' ); ?></option>
                            <option value="columns1" <?php echo ( $value == 'columns1' ) ? ' selected="selected" ' : ''; ?>><?php esc_html_e( '1 column Dropdown', 'avas-core' ); ?></option>
							<option value="columns2" <?php echo ( $value == 'columns2' ) ? ' selected="selected" ' : ''; ?>><?php esc_html_e( '2 columns Dropdown', 'avas-core' ); ?></option>
							<option value="columns3" <?php echo ( $value == 'columns3' ) ? ' selected="selected" ' : ''; ?>><?php esc_html_e( '3 columns Dropdown', 'avas-core' ); ?></option>
							<option value="columns4" <?php echo ( $value == 'columns4' ) ? ' selected="selected" ' : ''; ?>><?php esc_html_e( '4 columns Dropdown', 'avas-core' ); ?></option>
						</select>
					</label>
				</p>
				<?php
				...
				<p class="description description-thin">
					...
					<label for="edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>">
						<select id="edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>" class=" <?php echo esc_attr( $key ); ?>" name="<?php echo esc_attr( $key . "[" . $item_id . "]" ); ?>">
                            <option value="algcenter" <?php echo ( $value == 'algcenter' ) ? ' selected="selected" ' : ''; ?>><?php esc_html_e( 'Aligncenter Dropdown', 'avas-core' ); ?></option>
							<option value="algleft" <?php echo ( $value == 'algleft' ) ? ' selected="selected" ' : ''; ?>><?php esc_html_e( 'Alignleft Dropdown', 'avas-core' ); ?></option>
							<option value="algright" <?php echo ( $value == 'algright' ) ? ' selected="selected" ' : ''; ?>><?php esc_html_e( 'Alignright Dropdown', 'avas-core' ); ?></option>
							<option value="fullwidth" <?php echo ( $value == 'fullwidth' ) ? ' selected="selected" ' : ''; ?>><?php esc_html_e( 'Fullwidth Dropdown', 'avas-core' ); ?></option>
						</select>
					</label>
				</p>
				<?php
			...
			<p class="description description-thin"<?php echo esc_attr( $style ); ?>>
				...
				<label for="edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>">
					<select id="edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>" class=" <?php echo esc_attr( $key ); ?>" name="<?php echo esc_attr( $key . "[" . $item_id . "]" ); ?>">
						<option value="" <?php echo ( $value == '' ) ? ' selected="selected" ' : ''; ?>><?php esc_html_e( 'Select Widget Area', 'avas-core' ); ?></option>
						<?php
			...
			<p class="description description-thin">
				...
				<label for="edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>">
					<select id="edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>" class=" <?php echo esc_attr( $key ); ?>" name="<?php echo esc_attr( $key . "[" . $item_id . "]" ); ?>">
						<option value="0" <?php echo ( $value == '0' ) ? ' selected="selected" ' : ''; ?>><?php esc_html_e( 'No', 'avas-core' ); ?></option>
						<option value="1" <?php echo ( $value == '1' ) ? ' selected="selected" ' : ''; ?>><?php esc_html_e( 'Yes', 'avas-core' ); ?></option>
					</select>
				</label>
			</p>
		<?php }
        ...
        <p class="description description-thin">
            <label for="edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>">
                <?php echo esc_html( $title ); ?> <a href="<?php echo esc_url('https://fontawesome.com/v4.7.0/icons/'); ?>" target="_blank"><?php esc_html_e('Url Icon', 'avas-core'); ?></a><br />
            ...
            <p class="description description-wide">
                <label for="edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>">
                    <?php echo esc_html( $title ); ?><br />
                    <input type="text" value="<?php echo esc_attr( $value ); ?>" id="edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>" class=" <?php echo esc_attr( $key ); ?>" name="<?php echo esc_attr( $key . "[" . $item_id . "]" ); ?>" />
                    <button id="browse-edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>" class="set_custom_images button button-secondary submit-add-to-menu"><?php _e( 'Browse Image', 'avas-core' ); ?></button>
            ...
            <div class="menu-item-actions description-wide submitbox">
                <?php if( 'custom' != $item->type && $original_title !== false ) : ?>
                    <p class="link-to-original">
                        <?php printf( __('Original: %s', 'avas-core'), '<a href="' . esc_attr( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
                    </p>
                <?php endif; ?>
                ...
                ); ?>"><?php _e('Remove','avas-core'); ?></a> <span class="meta-sep"> | </span> <a class="item-cancel submitcancel" id="cancel-<?php echo esc_attr( $item_id ); ?>" href="<?php echo esc_url( add_query_arg( array('edit-menu-item' => $item_id, 'cancel' => time()), remove_query_arg( $removed_args, admin_url( 'nav-menus.php' ) ) ) );
                    ?>#menu-item-settings-<?php echo esc_attr( $item_id ); ?>"><?php _e('Cancel','avas-core'); ?></a>
            </div>
...
class HeroMenuWalker extends Walker_Nav_Menu {
    ...
    function start_lvl( &$output, $depth = 0, $args = array() )  {
		...
		$class .= 'depth'.$depth;
		$class .= ' '.$submenu_type;
        $class = $bg_image?$class .= ' sub-menu mega-bg-image':$class .= ' sub-menu elementor-nav-menu--dropdown';
        if ( $bg_image ) {
...
if(!function_exists('bddex_get_main_menu_parent_items')){
    function bddex_get_main_menu_parent_items(){
        $menu_name = 'main_menu';
        ...
        if ( isset( $locations[ $menu_name ] ) && $locations[ $menu_name ] != 0) {
            ...
            $items = bddex_get_menu_parent_items($menu_id);


            //get the WPML translated items
            $trans_items = bddex_get_translation_items($menu_id);
...
if(!function_exists('bddex_get_menu_parent_items')){
    function bddex_get_menu_parent_items($menu_id){
...
if(!function_exists('bddex_get_translation_items')){
    function bddex_get_translation_items($main_id){
        $items = array();
            ...
            foreach ($languages as $lang) {
                ...
                if(!empty($code)){
                    ...
                    if(!empty($menu_id_str)){
                        ...
                        if($menu_id!=$main_id){
                            $menu_items = bddex_get_menu_parent_items($menu_id);
